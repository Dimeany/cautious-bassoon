#ifndef _INTERFACE_DATABASE_L_THREAD_SAFE_QUEUE_H_
#define _INTERFACE_DATABASE_L_THREAD_SAFE_QUEUE_H_
...
public:
  ...
  void push(T t) {
    ...
    // thread-safe
    boost::asio::post(ctx, [this, t=std::move(t)]() {
  ...
  std::shared_ptr<PushDelayTimer> push_delay(T t, std::chrono::duration<Ref, Period> delay) {
    ...
    timer->boost_timer().async_wait([this, timer, t=std::move(t)](auto error_code) {
      if (error_code == boost::asio::error::operation_aborted) {
        ...
        throw ThreadSafeQueueException(error_code.what());
      }
